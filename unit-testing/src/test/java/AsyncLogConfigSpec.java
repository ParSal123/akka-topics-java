...
class AsyncLogConfigSpec {
  ...
  */
  ...
  public static final ActorTestKit testKit =
      ActorTestKit.create(
          ConfigFactory.parseString("akka.es-entity" + ".journal-enabled = false")
  ...
  void ActorMustLogInDebugContentWhenReceivingMessage() {
    ...
    LoggingTestKit.debug(String.format("message '%s', received", message))
        .expect(testKit.system(), tellHiToLoggerActor);
  }
  ...
  void ActorMustLogMessagesToDeadLetters() {
    ...
    LoggingTestKit.empty()
        ...
        .expect(testKit.system(), tellHelloToCarl);
  }
}

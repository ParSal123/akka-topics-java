...
class FishingSpec {
    ...
    void TimingTestMustBeAbleToCancelTimer() {
        ...
        var sender = Behaviors.<Sender.Command>withTimers(timer -> {
        ...
        probe.fishForMessage(java.time.Duration.ofSeconds(3), msg -> {
    ...
    void AutomatedResumingCounterMustReceiveResumeAfterPause() {
        ...
        var counterMonitored = Behaviors.monitor(CounterTimer.Command.class, probe.getRef(), CounterTimer.create());
...
class Sender {
    ...
    public static Behavior<Command> create(ActorRef<Receiver.Command> forwardTo, TimerScheduler<Command> timer) {
        return Behaviors.receiveMessage(cmd -> {

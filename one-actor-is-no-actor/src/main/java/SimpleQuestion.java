...
public class SimpleQuestion {
  public static void main(String[] args) {
    ActorSystem<Guardian.Command> guardian =
        ActorSystem.create(Guardian.create(), "example-ask-without-content");
                          ...
                          .forEach(
                              text -> {
                                ActorRef<Worker.Command> worker =
                                    context.spawn(
                                        Worker.create(text), String.format("worker-%s", text));
                                context.ask(
                                    Worker.Response.class,
                                    worker,
                                    ...
                                    (response, throwable) -> {
                                      if (throwable == null) {
                                        return new Report(
                                            String.format(
                                                "%s read by %s", text, worker.path().name()));
                                      } else {
                                        return new Report(
                                            String.format(

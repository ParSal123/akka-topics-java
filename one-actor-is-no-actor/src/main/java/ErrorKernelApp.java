...
class Manager {
    ...
    public static Behavior<Command> create() {
        return Behaviors.setup(context -> {
            ActorRef<Worker.Response> adapter = context.messageAdapter(Worker.Response.class, WorkerDoneAdapter::new);
            return BehaviorBuilder.<Command>create().onMessage(Delegate.class, delegate -> {
                ...
                return Behaviors.same();
            }).onMessage(WorkerDoneAdapter.class, workerDone -> {
                context.getLog().info("text '{}' has been finished", ((Worker.Done) (workerDone.response)).text());
                return Behaviors.same();
            }).build();
        });
    }
...
class Worker {
    ...
    public static Behavior<Command> create() {
        return Behaviors.setup(context -> BehaviorBuilder.<Command>create().onMessage(Parse.class, parse -> {
            String parsed = naiveParsing(parse.text);
            context.getLog().info("'{}' DONE!. Parsed result: {}", context.getSelf(), parsed);

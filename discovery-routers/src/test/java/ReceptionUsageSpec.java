...
class HotelConcierge {
    public static ServiceKey<VIPGuest.Command> GoldenKey = ServiceKey.create(VIPGuest.Command.class, "concierge-key");

    public static Behavior<Command> create() {
        return Behaviors.setup(context -> {
            var listingNotificationAdapter = context.messageAdapter(Receptionist.Listing.class, ListingResponse::new);

            context.getSystem().receptionist().tell(Receptionist.subscribe(GoldenKey, listingNotificationAdapter));

            return Behaviors.receiveMessage(message -> {
                if (message instanceof ListingResponse listingResponse) {
                    Set<ActorRef<VIPGuest.Command>> guests = listingResponse.listing.getServiceInstances(GoldenKey);
...
class GuestSearch {
    public static Behavior<Command> create(String actorName, ActorRef<ActorRef<VIPGuest.Command>> replyTo) {
        return Behaviors.setup(context -> {
            var listingNotificationAdapter = context.messageAdapter(Receptionist.Listing.class, ListingResponse::new);
            return Behaviors.receiveMessage(message -> {
                if (message instanceof Find) {
                    context.getSystem().receptionist()
                           .tell(Receptionist.find(HotelConcierge.GoldenKey, listingNotificationAdapter));
                    return Behaviors.same();
                    ...
                    guests.stream().filter(actor -> actor.path().name().contains(actorName)).forEach(replyTo::tell);
                    return Behaviors.same();
                } else {
                    return Behaviors.unhandled();
                }
...
class GuestFinder {
        ...
        return Behaviors.setup(context -> {
            ...
            return Behaviors.receiveMessage(message -> {
                if (message instanceof Find find) {
                    context.ask(Receptionist.Listing.class, context.getSystem().receptionist(), timeout,
                                replyTo -> Receptionist.find(HotelConcierge.GoldenKey, replyTo),
                                (response, throwable) -> {
                                    if (throwable == null) {
                                        Set<ActorRef<VIPGuest.Command>> guests =
                                                response.getServiceInstances(HotelConcierge.GoldenKey);
                                        guests.stream().filter(actor -> actor.path().name().contains(find.actorName()))
    ...
    public record Find(String actorName, ActorRef<ActorRef<VIPGuest.Command>> replyTo) implements Command {}
}